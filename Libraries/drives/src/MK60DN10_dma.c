...
void DMA_count_Init(DMA_CHn CHn, PTX_n ptxn, uint32 count, DMA_PORTx2BUFF_cfg cfg)
{
    ...
    DMA0->TCD[CHn].SOFF = 0;                            // 设置源地址偏移 = 0x0, 即不变
    ...
    GPIOX[(ptxn>>5)]->PDDR &= ~(uint32)(1<< (ptxn & 0x1f));                  //设置端口方向为输入
    ...
    GPIOX[(ptxn>>5)]->PDDR &= ~(uint32)(1<< (ptxn & 0x1f));                                                      //输入模式
