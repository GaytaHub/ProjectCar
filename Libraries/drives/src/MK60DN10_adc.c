...
void adc_start(ADCn_Ch adcn_ch, ADC_nbit bit)
{
    ...
    ADCX[adcn]->CFG1 = (0
                        ...
                        | ADC_CFG1_MODE(bit) | ADC_CFG1_ADICLK(0) //0为总线时钟,1为总线时钟/2,2为交替时钟（ALTCLK），3为 异步时钟（ADACK）。
    );
    ...
    //写入 SC1A 启动转换
    ADCX[adcn]->SC1[0] = (0 | ADC_SC1_AIEN_MASK // 转换完成中断,0为禁止，1为使能
...
//多次采集求平均值
#define Times 5
float adc_average(ADCn_Ch adcn_ch, ADC_nbit bit)
{
    int8 i;
    uint16 ADC_Value[Times] = {0};
    float Result = 0.0;

    for (i = 0; i < Times; i++)
    {
        ADC_Value[i] = adc_once(adcn_ch, bit);
        Result += ADC_Value[i];
    }
    Result = Result / Times;
